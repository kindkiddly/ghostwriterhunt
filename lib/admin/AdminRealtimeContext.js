...
export function AdminRealtimeProvider({ children }) {
  ...
  const [callbackPendingCount, setCallbackPendingCount] = useState(0);
  ...
  const refresh = useCallback(async () => {
    const [{ data, error }, { count: pendingCallbacks }] = await Promise.all([
      supabase
    ...
    setCallbackPendingCount(pendingCallbacks ?? 0);
    setLoading(false);
      ...
      .on(
        ...
        (payload) => {
          if (
            ...
            payload.new?.callback_status === "pending" &&
            payload.old?.callback_status !== "pending" &&
